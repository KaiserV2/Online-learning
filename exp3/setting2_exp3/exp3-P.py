...
def normalize_data(lines):
    ...
    new_lines = lines / row_sums[: ,np.newaxis]

    return new_lines.tolist()
...
def Exp3p(loss_data, T, K, etha, beta, gamma):
    P = np.ones( K ) / K
    estimated_loss = np.ones( K ) / K
    cumulative_loss = 0
    avg_cum_loss = []
    Cum_loss = np.zeros(K)

    norm_loss = normalize_data(loss_data)
    ...
    for t in range( T ):
        ...
        norm_loss_data = norm_loss[t]

        # Choose arm with P
        arm_index = np.random.choice( list( range( K ) ) ,p=P )
        cumulative_loss +=  ArmLossVector[arm_index]
        avg_cum_loss.append( cumulative_loss / (t + 1) )

        # exp3 update
        for i in range( 10 ):
            if i == arm_index:
                estimated_loss[i] = (norm_loss_data[i] + beta) / P[i]
            else:
                estimated_loss[i] =  beta / P[i]

        for i in range(10):
            Cum_loss[i] += estimated_loss[i]

        P = (1 - gamma) * ((np.exp( -etha * Cum_loss )) / (np.sum( np.exp( -etha * Cum_loss ) )))  + gamma / K
        ...
        if t == 4999:     #4
            ...
            plt.plot( range( 10 ) ,plot_freq ,color='red' ,label='exp3-P' )
            plt.legend()
            plt.title( "xt_exp3-P 5000 iteration" )
            plt.tight_layout()
            plt.savefig('../figs/exp3-P/exp3p_dynamic_5000.png', dpi=250)
            plt.show()
        ...
        if t == 9999:   #0
            ...
            plt.plot( range( 10 ) ,plot_freq ,color='red' ,label='exp3-P' )
            plt.legend()
            plt.title( "xt_exp3-P 10000 iteration" )
            plt.tight_layout()
            plt.savefig( '../figs/exp3-P/exp3p_dynamic_10000.png' ,dpi=250 )
            plt.show()
        ...
        if t == 14999:   #0, 7
            ...
            plt.plot( range( 10 ) ,plot_freq ,color='red' ,label='exp3-P' )
            plt.legend()
            plt.title( "xt_exp3-P 15000 iteration" )
            plt.tight_layout()
            plt.savefig( '../figs/exp3-P/exp3p_dynamic_15000.png' ,dpi=250 )
            plt.show()
        ...
        if t == 19999: #1, 7
            ...
            plt.plot( range( 10 ) ,plot_freq ,color='red' ,label='exp3-P' )
            plt.legend()
            plt.title( "xt_exp3-P 20000 iteration" )
            plt.tight_layout()
            plt.savefig( '../figs/exp3-P/exp3p_dynamic_20000.png' ,dpi=250 )
            plt.show()
...
#     gamma = 0.01
#     #gamma = K * etha
#     R, avg_loss = Exp3p( loss_data_static, T ,K , etha, beta, gamma )
#     all_etha_loss.append(avg_loss)
#
# plt.loglog( range(T) ,all_etha_loss[0] ,color='b', linestyle='-', label='etha=0.0001')
...
#
# plt.title( "Static dist: exp3-P")
# plt.xlabel( "time" )
# plt.ylabel( "average cumulative loss" )
# plt.legend()
# plt.tight_layout()
# plt.savefig('../figs/exp3-P_static.png', dpi=300)
# plt.show()
# #
...
#     beta = etha
#     gamma = K * etha
#     R, avg_loss = Exp3p( loss_data_dynamic, T ,K , etha,  beta, gamma )
#     all_etha_loss.append(avg_loss)
#
...
# plt.loglog( range(T) ,all_etha_loss[4] ,color='m', linestyle='-', label='etha=0.03')
#
# plt.title( "Dynamic dist: exp3-P")
# plt.xlabel( "time" )
# plt.ylabel( "average cumulative loss" )
# plt.legend()
# plt.tight_layout()
# plt.savefig('../figs/exp3-P_dynamic.png', dpi=300)
# plt.show()
